...
function saveaio() {
  ...
  if (code = localStorage.getItem('codeaio').length != 0) {
    ...
    <button class='bx bxs-copy-alt' onclick="let codes1 = document.createElement('textarea');codes1.value = code = localStorage.getItem('codeaio').value;document.body.appendChild(codes1);codes1.select();document.execCommand('copy');document.body.removeChild(codes1); swal('Copied!', 'Code is copied to your clipboard.', 'success')"> Copy Code</button>
    </div>
    <br>
    `.replace('null',''))
    swal({
